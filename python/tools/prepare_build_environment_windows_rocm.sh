...
# Check if Intel MKL is already installed (avoids massive download)
if [ ! -d "C:/Program Files (x86)/Intel/oneAPI/mkl" ]; then
...
if [ ! -f "$DEPS_INSTALL_DIR/lib/dnnl.lib" ]; then
    NPROC=$(nproc)
...
# Enable ccache for the main build
export CMAKE_C_COMPILER_LAUNCHER=ccache
export CMAKE_CXX_COMPILER_LAUNCHER=ccache

cmake -GNinja -DCMAKE_BUILD_TYPE=Release -S . -B build -DCMAKE_CXX_FLAGS="-Wno-deprecated-literal-operator" -DCMAKE_HIP_FLAGS="-Wno-deprecated-literal-operator" -DCMAKE_INSTALL_PREFIX=$CTRANSLATE2_ROOT -DCMAKE_PREFIX_PATH="C:/Program Files (x86)/Intel/oneAPI/compiler/latest/lib;$DEPS_INSTALL_DIR" -DBUILD_CLI=OFF -DWITH_DNNL=ON -DWITH_HIP=ON -DCMAKE_HIP_ARCHITECTURES="$PYTORCH_ROCM_ARCH"
